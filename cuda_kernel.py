...
@cuda.jit(
    "void(Array(float64, 2, 'C'), Array(float64, 1, 'C'), Array(float64, 2, 'F'), int32, Array(float64, 2, 'C'))"
)
...
def svd_reco_cuda(
    ...
    vt = np.asfortranarray(vt) # make column major
    ...
    y = cuda.device_array((m, n), dtype=np.float64 if fp64 else np.float32)
